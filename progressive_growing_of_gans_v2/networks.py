...
def get_weight(shape, gain=np.sqrt(2), use_wscale=False, fan_in=None):
    ...
    if use_wscale:
        ...
        #v1 code
        return tf.compat.v1.get_variable('weight', shape=shape, initializer=tf.compat.v1.initializers.random_normal(), use_resource=False) * wscale
    else:
        #v1 code
        return tf.compat.v1.get_variable('weight', shape=shape, initializer=tf.compat.v1.initializers.random_normal(0, std), use_resource=False)
...
def apply_bias(x):
    #v1 code
    b = tf.compat.v1.get_variable('bias', shape=[x.shape[1]], initializer=tf.compat.v1.initializers.zeros(), use_resource=False)
    b = tf.cast(b, x.dtype)
    if len(x.shape) == 2:
        return x + b
    else:
...
def G_paper(
    ...
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    #v1 code
    lod_in = tf.cast(tf.compat.v1.get_variable('lod', initializer=np.float32(0.0), trainable=False, use_resource=False), dtype)
