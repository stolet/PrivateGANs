...
#----------------------------------------------------------------------------
# Differentially private wrapper for tf.train.Optimizer that automatically takes care of:
...
class DPOptimizer:
    def __init__(
        ...
        minibatch_split,
        name                = 'Train',
        tf_optimizer        = 'tensorflow_privacy.privacy.optimizers.dp_optimizer.DPAdamGaussianOptimizer',
        learning_rate       = 0.001,
    ...
    def register_gradients(self, loss, vars):
        ...
        with tf.name_scope(self.id + '_grad'), tf.device(dev):
            if dev not in self._dev_opt:
                ...
                self._dev_opt[dev] = self.optimizer_class(name=opt_name, learning_rate=self.learning_rate, l2_norm_clip=self.l2_norm_clip, noise_multiplier=self.noise_multiplier, num_microbatches=self.num_microbatches, **self.optimizer_kwargs)
                self._dev_grads[dev] = []
